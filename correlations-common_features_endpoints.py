...
main_dir = Path('the path of main folder')


#read the features from Excel files
df_pyrads_data = pd.read_excel(os.path.join(/clinical_IHC_individual_inter_with_rads_bw25.xlsx'), sheet_name='pyrads-common-racat' )
df_IHC_data = pd.read_excel(os.path.join(main_dir, '/clinical_IHC_individual_inter_with_rads_bw25.xlsx'), sheet_name='IHC' )
df_racat_data = pd.read_excel(os.path.join(main_dir, '/clinical_IHC_individual_inter_with_rads_bw25.xlsx'), sheet_name='racat-common-pyrads')  
df_features = pd.read_excel(os.path.join(main_dir, '/clinical_IHC_individual_inter_with_rads_bw25.xlsx'), sheet_name='common-features')  
...
# Loop through each column index and calculate Spearman correlation between each radiomic feature in RaCat and the endpoint
...
writer = pd.ExcelWriter(os.path.join(main_dir, '/evaluators_common_features_pyrads_CD8_new.xlsx'), engine='xlsxwriter')
wb  = writer.book
df_pyrads.to_excel(writer)    ## write into excel
wb.close()

writer = pd.ExcelWriter(os.path.join(main_dir, '/evaluators_common_features_racat_CD8_new.xlsx'), engine='xlsxwriter')
wb  = writer.book

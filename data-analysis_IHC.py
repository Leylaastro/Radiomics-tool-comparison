...
writer = pd.ExcelWriter(os.path.join(main_dir,'Results/data_analysis/Pyrads/clinical_IHC_individual_inter_with_rads.xlsx'), engine='xlsxwriter')
wb  = writer.book
...
writer = pd.ExcelWriter(os.path.join(main_dir, 'Results/data_analysis/Pyrads/clinical_IHC_rads_bw25_IUCPQ_intersection.xlsx'), engine='xlsxwriter')
wb  = writer.book
...
rad_data_intersected_total.to_excel(writer, sheet_name="radiomics_pyrads_bw25")    ## write into excel

wb.close()

"""

#endpoint_label = ["CD8_tumeur",'CD163_tumeur', "CD3_tumeur", 'CD4_tumeur', 'PD1_tumeur',  'TIM3_tumeur', 'Lag3_tumeur', 'CD56_tumeur']
#endpoint_label = ['TIM3_tumeur', 'Lag3_tumeur', 'CD56_tumeur']

endpoint_label = ["pfs_days"]
...
#	trg = IHC_data_intersected_total [label]

	trg = clinical_data_intersected_total [label]
	for ftr in rad_data_intersected_total.columns:
#		print(label)
		corr_prs = scipy.stats.pearsonr(rad_data_intersected_total[ftr], trg)    # Pearson's r
		corr_spr = scipy.stats.spearmanr(rad_data_intersected_total[ftr], trg)   # Spearman's rho
...
#	writer = pd.ExcelWriter(os.path.join(main_dir, 'Results/data_analysis/Pyrads/correlation_rads_IHC_bw25_'+label+'.xlsx'), engine='xlsxwriter')
#	wb  = writer.book
#	df = pd.DataFrame(corr_prs_total, columns = ["corr_prs"], index = rad_data_intersected_total.columns)     ## put into a dataframe format
...
#	print(feature_importances[0])
	feature_importances.nlargest(len(rad_data_intersected_total.columns))
#	print(feature_importances.nlargest(2))
	rank_features = feature_importances.rank(ascending=False)

	writer = pd.ExcelWriter(os.path.join(main_dir, 'Results/data_analysis/Pyrads/rank_rads_bw25_prs_'+label+'.xlsx'), engine='xlsxwriter')
	wb  = writer.book
	rank_features.to_excel(writer, sheet_name="rank_rads_bw25_"+label)    ## write into excel 
	wb.close()
...
"""
	...
	plt.title("The importance of the features for " +label+ " prediction" +" from Pyrads with binwidth = 25", fontdict={'fontsize':80}, y= 1.02)
	plt.savefig(os.path.join(main_dir, "importance_cc_pear_Pyrads_bw25_"+label +"_.png"))
"""
exit()
...
writer = pd.ExcelWriter(os.path.join(main_dir,'Results/data_analysis/Pyrads/radiomicsfeatures_pyrads_bw25_clean_common_with_iucpq_radiomics_labels.xlsx'), engine='xlsxwriter')
wb  = writer.book
...
writer = pd.ExcelWriter(os.path.join(main_dir,'Results/data_analysis/Pyrads/radiomicsfeatures_pyrads_bw25_clean.xlsx'), engine='xlsxwriter')
wb  = writer.book
